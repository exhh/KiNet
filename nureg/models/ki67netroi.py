...
class Ki67NetRoi(nn.Module):
    def __init__(self, nll=False):
        super(Ki67NetRoi, self).__init__()
        ...
        self.up_tr1_ROI   = UpConcat(256, 256, 512, 2,  dropout=True)
        self.up_tr2_ROI   = UpConcat(512, 128, 256, 2, dropout=True)
        self.up_tr3_ROI   = UpConcat(256, 64, 128, 1)
        self.up_tr4_ROI   = UpConcat(128, 32, 64, 1)
    ...
    def forward(self, x):
        ...
        out_up1_ROI = self.up_tr1_ROI(out256, out128)
        out_up2_ROI = self.up_tr2_ROI(out_up1_ROI, out64)
        out_up3_ROI = self.up_tr3_ROI(out_up2_ROI, out32)
        out_up4_ROI = self.up_tr4_ROI(out_up3_ROI, out16)

        out_up5_ROI = self.up1_ROI(out_up1_ROI, x.size()[2:])
        out_up6_ROI = self.up2_ROI(out_up2_ROI, x.size()[2:])
        out_ROI = torch.cat([out_up4_ROI, out_up5_ROI, out_up6_ROI], 1)
        out_ROI = self.out_tr_ROI(out_ROI)
    ...
    def predict(self, batch_data, batch_size=None):
        ...
        if batch_size is None or batch_size >= total_num:
            ...
            det, roi = self.forward(x)
            return [det.cpu().data.numpy(), roi.cpu().data.numpy()]
        else:
            results, roi_results = [], []
            for ind in Indexflow(total_num, batch_size, False):
                data = batch_data[ind]
                data = to_device(data, self.device_id, False).float()
                det, roi = self.forward(data)
                results.append(det.cpu().data.numpy())
                roi_results.append(roi.cpu().data.numpy())
            return [np.concatenate(results,axis=0), np.concatenate(roi_results,axis=0)]

@register_model('ki67netroi')
def ki67netroi(pretrained=True, finetune=False, out_map=True, **kwargs):
    model = Ki67NetRoi()
    return model
