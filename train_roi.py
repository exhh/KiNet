...
def binary_crossentropy(y_true, y_pred):
    #y_pred should be pre_softmax: N x 1 x W x H
    sumit = -1.0 * y_true*torch.log(torch.sigmoid(y_pred) + 1e-10) - (1-y_true) * torch.log(1-torch.sigmoid(y_pred) + 1e-10)
...
@click.option('--use_roi/--no-use_roi', default=False)
@click.option('--det_weight', default=25.0)
def main(output, dataset, datadir, batch_size, lr, step, iterations,
        ...
        weights, model, gpu, num_cls, use_roi, det_weight):
    ...
    transform = []
    target_transform = []
    datasets = [get_dataset(name, os.path.join(datadir,name), use_roi=use_roi)
                for name in dataset]
    ...
    for im, label in roundrobin_infinite(*loaders):
        ...
        label_scale, label_mask = get_weight_mask(label[:,:-1,:,:], params)
        label_v = make_variable(label_scale, requires_grad=False)
        label_mask_v = make_variable(label_mask, requires_grad=False)

        roi_v = label[:,-1:,:,:] / 255.0
        roi_v = make_variable(roi_v, requires_grad=False)

        preds, roi_preds = net(im_v)
        loss = weighted_loss(label_v, preds, label_mask_v)
        roi_loss = binary_crossentropy(roi_v, roi_preds)
        total_loss = det_weight * loss + roi_loss
        total_loss_val = total_loss.data.cpu().numpy().mean()

        assert not np.isnan(total_loss_val) ,"nan error"
        steps.append(iteration)
        vals.append(total_loss_val)
